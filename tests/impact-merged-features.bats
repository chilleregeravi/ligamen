#!/usr/bin/env bats
#
# impact-merged-features.bats — /11/12/13 serialization guard.
...
# with grep fallback) BEFORE  deletes commands/cross-impact.md.
#
...
@test "/arcanon:impact advertises --exclude in argument-hint" {
  [ -f "$IMPACT_MD" ]
  run grep -E '^argument-hint:.*--exclude' "$IMPACT_MD"
  [ "$status" -eq 0 ]
}

@test "/arcanon:impact documents --exclude filter for graph results" {
...
@test "/arcanon:impact documents --exclude filter for grep results" {
...
@test "/arcanon:impact advertises --changed in argument-hint" {
  run grep -E '^argument-hint:.*--changed' "$IMPACT_MD"
  [ "$status" -eq 0 ]
}

@test "/arcanon:impact --changed auto-detects from git diff HEAD" {
  grep -q 'git diff --name-only HEAD' "$IMPACT_MD"
}

@test "/arcanon:impact treats no-args invocation as --changed" {
...
@test "/arcanon:impact documents 3-state degradation model" {
...
@test "State A falls back to scripts/impact.sh grep scanner" {
  grep -q 'scripts/impact.sh' "$IMPACT_MD"
  grep -Eq -i 'legacy.*fallback|legacy.*scan' "$IMPACT_MD"
}

@test "State B prompts user to run /arcanon:map" {
...
@test "State C preserves MCP-first graph query flow" {
...
@test "/arcanon:impact never starts the worker (query-only)" {
...
@test "original --direction and --hops flags preserved (not regressed)" {
...
@test "frontmatter allowed-tools covers Bash + MCP + AskUserQuestion" {
