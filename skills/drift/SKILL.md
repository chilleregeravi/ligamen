...
Sibling repos: !`source "${CLAUDE_PLUGIN_ROOT}/lib/siblings.sh" && list_siblings "${CLAUDE_PLUGIN_ROOT}"`
...
2. For `versions` (or no subcommand specified):
   Run: `"${CLAUDE_PLUGIN_ROOT}/skills/drift/scripts/drift-versions.sh" $ALLCLEAR_ARGS`

3. For `types` (or no subcommand specified):
   Run: `"${CLAUDE_PLUGIN_ROOT}/skills/drift/scripts/drift-types.sh" $ALLCLEAR_ARGS`

4. For `openapi` (or no subcommand specified):
   Run: `"${CLAUDE_PLUGIN_ROOT}/skills/drift/scripts/drift-openapi.sh" $ALLCLEAR_ARGS`
...
- If no sibling repos are found, the skill exits with a helpful message
