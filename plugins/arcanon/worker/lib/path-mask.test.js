 ...
 *   6  — maskHome — idempotent on already-relative paths 
 ...
 *  10  — maskHomeDeep — masks raw string values regardless of key 
...
test('maskHome — non-string input passes through', () => {
...
test('maskHome — idempotent on already-relative paths (S1 mitigation)', () => {
...
test('maskHome — idempotent on already-masked paths', () => {
...
test('maskHomeDeep — nested object walk, no input mutation', () => {
...
test('maskHomeDeep — nested array walk', () => {
  const input = [{ path: FAKE_HOME + '/a' }, { path: FAKE_HOME + '/b' }];
  const out = maskHomeDeep(input);
  assert.deepEqual(out, [{ path: '~/a' }, { path: '~/b' }]);
});

test('maskHomeDeep — masks raw string values regardless of key (M1 mitigation)', () => {
...
test('maskHomeDeep — cycle safety (no infinite loop, no throw)', () => {
  const a = {};
  a.self = a;
...
test('maskHomeDeep — non-object passes through maskHome', () => {
