...
describe('isFirstScan()', () => {
  ...
  it('exports isFirstScan function', () => {
    assert.strictEqual(typeof importedDb.isFirstScan, 'function', 'isFirstScan must be exported');
  });

  it('returns true before any snapshots exist', () => {
    const result = importedDb.isFirstScan();
    assert.strictEqual(result, true, 'isFirstScan must return true when map_versions is empty');
  });
});
...
describe('createSnapshot()', () => {
  ...
  it('exports createSnapshot function', () => {
    assert.strictEqual(typeof importedDb2.createSnapshot, 'function', 'createSnapshot must be exported');
  });

  it('returns a path to a .db file that exists after creation', () => {
    const snapshotPath = importedDb2.createSnapshot('test-label');
    assert.ok(typeof snapshotPath === 'string', 'createSnapshot must return a string path');
    assert.ok(snapshotPath.endsWith('.db'), 'snapshot path must end with .db');
    assert.ok(fs.existsSync(snapshotPath), `snapshot file must exist at: ${snapshotPath}`);
  });

  it('isFirstScan returns false after a snapshot exists', () => {
    const result = importedDb2.isFirstScan();
    assert.strictEqual(result, false, 'isFirstScan must return false after a snapshot exists');
  });
  ...
  it('snapshot file is in a snapshots/ subdirectory', () => {
    ...
    assert.strictEqual(snapshotDirName, 'snapshots', 'snapshot must be in a snapshots/ subdirectory');
  });
  ...
  it('snapshot is recorded in map_versions table', () => {
    ...
    assert.ok(rows.length >= 2, `map_versions must have at least 2 rows, got ${rows.length}`);
    assert.ok(rows[0].snapshot_path, 'snapshot_path must be set in map_versions');
    assert.ok(rows[0].snapshot_path.startsWith('snapshots/'), 'snapshot_path must be relative (starts with snapshots/)');
  });
});
...
describe('createSnapshot() retention cleanup', () => {
  ...
  it('retains at most 10 snapshots by default when creating 12 snapshots', async () => {
    ...
    const rows = db.prepare('SELECT * FROM map_versions ORDER BY created_at ASC').all();
    assert.ok(rows.length <= 10, `Expected at most 10 rows in map_versions after cleanup, got ${rows.length}`);

    // The 2 oldest snapshot files should NOT exist
    assert.ok(!fs.existsSync(paths[0]), `Oldest snapshot file should have been deleted: ${paths[0]}`);
    assert.ok(!fs.existsSync(paths[1]), `Second oldest snapshot file should have been deleted: ${paths[1]}`);

    // The most recent snapshots should still exist
    const lastPath = paths[paths.length - 1];
    assert.ok(fs.existsSync(lastPath), `Most recent snapshot must still exist: ${lastPath}`);
  });
});
