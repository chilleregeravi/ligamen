...
const hitAfterDelete = db.prepare("SELECT name FROM services_fts WHERE services_fts MATCH 'payment'").pluck().all();
assert.ok(!hitAfterDelete.includes('payment-service'), 'FTS5 no longer has deleted service');

// All 9 FTS5 triggers exist
const triggers = db.prepare("SELECT name FROM sqlite_master WHERE type='trigger'").pluck().all();
const expectedTriggers = [
  'services_ai', 'services_ad', 'services_au',
  'connections_ai', 'connections_ad', 'connections_au',
  'fields_ai', 'fields_ad', 'fields_au',
];
