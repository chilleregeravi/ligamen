...
function minimalValid() {
  return {
    ...
    confidence: 'high',
    services: [
      { name: 'test-svc', root_path: 'src/', language: 'typescript', confidence: 'high' }
    ],
    connections: [],
    schemas: []
  };
...
test('validateFindings returns valid:false for empty object (missing connections)', () => {
  ...
  assert.equal(result.valid, false);
  assert.ok(result.error.includes('connections'), `Expected error about 'connections', got: ${result.error}`);
});
...
test('validateFindings returns valid:false when confidence is invalid value', () => {
  ...
  const result = validateFindings(obj);
  assert.equal(result.valid, false);
  assert.ok(result.error.includes('high') || result.error.includes('low'), `Expected error about confidence values, got: ${result.error}`);
});
...
test('validateFindings returns valid:false for invalid connection confidence', () => {
  ...
  assert.equal(result.valid, false);
  assert.ok(
    result.error.includes('confidence') || result.error.includes('high') || result.error.includes('low'),
...
test('validateFindings returns valid:false for invalid schema role', () => {
  ...
  assert.equal(result.valid, false);
  assert.ok(
    result.error.includes('role') || result.error.includes('request') || result.error.includes('event_payload'),
...
test('parseAgentOutput returns valid:false with error when no JSON block found', () => {
  const result = parseAgentOutput('I found some services but here is prose only.');
  assert.equal(result.valid, false);
...
test('parseAgentOutput returns valid:false with JSON parse error for malformed JSON', () => {
  ...
  const result = parseAgentOutput(raw);
  assert.equal(result.valid, false);
  assert.ok(result.error.startsWith('JSON parse error:'), `Expected JSON parse error, got: ${result.error}`);
});
...
test('parseAgentOutput validates the extracted JSON against the schema', () => {
  ...
  assert.ok(!result.error.startsWith('JSON parse error:'), `Should fail schema validation, got: ${result.error}`);
});
