...
6. **Extract image tags.** Compare expected image tags from the overlay against tags running in the cluster.
...
8. **Report results.** Display the following sections:
   ...
   **Read-only notice:** This command performs verification only. No changes have been applied to the cluster. Never run `kubectl apply` — this command is read-only.
